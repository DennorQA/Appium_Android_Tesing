...
class Test_Chapter5:
    ...
    def test_intent_with_results_text_is_displayed(self):
        ...
        main_page.tap_chapter5(
            main_page.chapter5_field)
        main_page.tap_intent_with_results(
            main_page.intent_with_results_field)
        main_page.tap_launch_activity_for_result(
            main_page.launch_activity_for_result_field)
        main_page.send_keys(
            main_page.text_input_field, Config.INPUT_TEXT)
        main_page.tap_send_result(
            main_page.send_result_button_field)
        actual_text = main_page.get_element_text(
            main_page.result_field)
